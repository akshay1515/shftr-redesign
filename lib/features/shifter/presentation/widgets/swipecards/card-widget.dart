...
class _CardWidgetState extends State<CardWidget> {
  ...
  Widget StatusStamps() {
    ...
    switch (status) {
      case Jobstatus.accepted:
        final child =
            buildMessage(color: Colors.green, text: "Accept", angle: -0.5,opacity:opacity);
        return Positioned(
            top: 64,
            left: 50,
            child: child);
      case Jobstatus.rejected:
        final child =
        buildMessage(color: Colors.red, text: "Reject", angle: 0.5,opacity:opacity);
        return Positioned(
            top: 64,
            right: 50,
            child: child);
      case Jobstatus.saved:
        final child =
       Center(child:  buildMessage(color: Colors.amber.shade700, text: "Save", angle: -0.5,opacity:opacity));
        return Positioned(
            bottom: 128,
            right: 0,
            left: 0,
  ...
  Widget buildMessage(
      {required Color color, required String text, double angle = 0,required double opacity}) {
    return Opacity(
      opacity: opacity,
